...
async function main() {
  try {
    // ✅ Подключаемся к БД
    // ВАЖНО: если у тебя MONGO_URI без dbname, то добавляем /travelPlannerDB
    // Если в .env уже стоит URI с /travelPlannerDB — тогда не добавляй второй раз.
    ...
    const uri =
      uriBase.includes("/travelPlannerDB")
        ? uriBase
        : uriBase.replace(/\/$/, "") + "/travelPlannerDB";

    await mongoose.connect(uri);
    console.log("✅ Connected to MongoDB:", uri.includes("@") ? uri.split("@")[1] : uri);

    // ✅ Найдём админа (которого создаёт ensureAdmin) или того кто в env
    const adminUsername = process.env.ADMIN_USERNAME || "admin";
    const admin = await User.findOne({ username: adminUsername }).select("_id username role");

    if (!admin) {
      console.error(
        `❌ Admin user not found: "${adminUsername}".\n` +
        `Create admin first (ensureAdmin) or register "${adminUsername}".`
      );
      process.exit(1);
    }
    ...
    // ✅ Реалистичные 20 записей + ownerId
    const records = [
      {
    ...
    ].map(d => ({ ...d, ownerId: admin._id })); // ✅ добавляем ownerId всем

    await Destination.insertMany(records);
    console.log(`✅ Inserted ${records.length} destinations with ownerId = ${admin.username}`);
    process.exit(0);
  } catch (err) {
